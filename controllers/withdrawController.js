...
// Controller: Group payout withdrawal
const withdrawGroupPayout = async (req, res) => {
  ...
  try {
    ...
    // Get CSRF token
    const csrfToken = await getCsrfToken(process.env.ROBLOX_COOKIE);
    ...
    const payoutRes = await axios.post(
      `https://groups.roblox.com/v1/groups/${process.env.GROUP_ID}/payouts`,
      payoutPayload,
      {
        headers: {
    ...
    res.json({ success: true, message: 'Payout successful!', data: payoutRes.data });
  } catch (err) {
